...
from transformers import Qwen2_5_VLForConditionalGeneration, AutoProcessor  # // 修改点
...
def load_model_and_processor():
    logger.info("正在加载Qwen2.5-VL-7B-Instruct模型...")  # // 修改点（日志）

    # 本地权重路径：如果你是本地落盘，请改成你的实际目录；
    # 也可直接用 HF Hub 名称 "Qwen/Qwen2.5-VL-7B-Instruct"
    model_name = "/home/guohaiyun/yangtianyu/CPRCIR/checkpoints/hf_models/Qwen2.5-VL-32B-Instruct"  # // 修改点

    model = Qwen2_5_VLForConditionalGeneration.from_pretrained(  # // 修改点（类名）
        model_name,
    ...
    try:
        ...
        ##Global rules (read carefully)
            ...
            - Grounding:
                ...
                - Relative requirements compare REFERENCE vs TARGET (e.g., “different breed”, “face the other direction”).
