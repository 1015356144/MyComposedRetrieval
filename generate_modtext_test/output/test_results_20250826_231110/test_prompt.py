...
def construct_prompt(reference_image_path, hard_negative_image_path, modification_text):
    ...
    - Output exactly these 6 sections, in this order, with the exact section headers shown below.
    ...
    ##Decompose intents:
    - One line per atomic intent, covering all content-bearing phrases in the input text (split by commas/“and/then”/parentheses).
    - Format (one per line):
    ...
    - Answer each question (same IDs and order) with Yes/No/Uncertain and brief evidence.
    - Format (one per line):
        I# | A=Yes|No|Uncertain | evidence_target="..." | evidence_ref="..."
    - Notes:
        - If scope=target_only, fill evidence_ref="".
        - If scope=compare_ref_target, cite both evidence_target and evidence_ref.
        - If visibility is insufficient, choose Uncertain.

    ##Local edits:
    - Map each intent to a local edit: Yes → keep, No → rewrite to match TARGET, Uncertain → generalize/delete.
    - Format (one per line):
    ...
    Example format:
        ...
        Decompose intents:
        I1 | span="..." | intent="..." | type=absolute | objects=[...] | note="..."
        I2 | span="..." | intent="..." | type=relative | objects=[...] | note="..."

        Generate polar questions:
        I1 | scope=target_only | Q="In the TARGET image, is ... ?"
        I2 | scope=compare_ref_target | Q="Comparing REFERENCE and TARGET, does ... ?"

        Answer questions via image comparison:
        I1 | A=Yes | evidence_target="..." | evidence_ref=""
        I2 | A=No | evidence_target="..." | evidence_ref="..."

        Local edits:
        I1 | action=keep | before="..." | after="..." | reason="..."
        I2 | action=rewrite | before="..." | after="..." | reason="..."
