...
def construct_prompt(reference_image_path, hard_negative_image_path, modification_text):
    tem_prompt="""You are a multimodal edit auditor. You get two images: FIRST=REFERENCE, SECOND=TARGET and an input edit text. Your only goal: minimally rewrite the edit text so it is true in the TARGET.

Hard rules (read first):

Use TARGET-only evidence for absolute intents; use REFERENCE only for relative intents.

QA is intent-centered: produce exactly one QA per intent (|qa|==|intents|, same order, same id).

Emit only facts needed to test these intents (no orphan/duplicate facts; ≤8 facts).

Keep the output to one JSON object; first char {, last char }.

Stage 0 — Captions (short, literal; no speculation)

captions.reference: start with [REFERENCE], 2–3 short sentences.

captions.target: start with [TARGET], 2–3 short sentences.

Stage 1 — Intent decomposition (from text only)

Split into atomic intents (color/count/add/remove/pose/layout/material/style/text/logo).

For each: id, span, intent, type(absolute|relative), objects.

Stage 2 — Intent typing

absolute = verifiable from TARGET alone; relative = needs REFERENCE vs TARGET.

Stage 3.0 — Plan (intent→predicates)

For each intent I#, list minimal TARGET predicates to verify it, e.g. C#.category, C#.pose, C#.relation(front_of)=C#.

You may only emit facts required by this plan (+ minimal location descriptors).

Stage 3 — Target facts (≤8, canonical)

Enumerate objects as C1..C6, then “… and others (not enumerated)”.

Fact form: "<C#>.<property|relation>=<value|OtherC#>".

No duplicates; each fact must be referenced by some QA via fact_ids.

Stage 4 — QA (intent-aligned; yes/no only)

One QA per intent; same id and order.
...
Allowed auxiliaries only: is/are/does/do/has/have/can/was/were/did.

No wh-words (“how/what/which/…”).

Include fact_ids referencing the minimal facts; if none, [] and answer “Uncertain”.

Stage 5 — Answers + evidence

Answer each QA with Yes/No/Uncertain; add evidence_target (and evidence_ref if relative).

Evidence must be consistent with fact_ids.

Stage 6 — Local edits → Stage 7 — text_new

Map Yes=keep, No=rewrite, Uncertain=generalize/delete.

Synthesize a concise text_new that is directly supported by TARGET.

JSON schema (single object):
{
"options_used": {},
"captions": {"reference": "", "target": ""},
"intents": [{"id":"I1","span":"...","intent":"...","type":"absolute|relative","objects":["..."]}],
"target_facts": [{"id":"F1","fact":"...","evidence_target":"..."}],
"qa": [{"id":"I1","scope":"target_only|compare_ref_target","question":"...","answer":"Yes|No|Uncertain","evidence_target":"...","evidence_ref":"","fact_ids":["F1"]}],
"local_edits": [{"id":"I1","action":"keep|rewrite|generalize|delete","before":"...","after":"...","reason":"..."}],
"text_new":"..."
}
