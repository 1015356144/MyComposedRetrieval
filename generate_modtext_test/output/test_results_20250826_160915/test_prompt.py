...
Stage 4 — Sub-questions (intent-anchored)
- Generate exactly ONE sub-question per intent (same order as in "intents").
- `id` of each QA MUST equal the intent id (e.g., I3).
- Absolute: start the question with “In the TARGET image, …”. Relative: start with “Comparing REFERENCE and TARGET, …”.
- Each QA must include `fact_ids`: the minimal set of F# that support the answer. If no target evidence, leave `fact_ids`=[] and answer "Uncertain".
