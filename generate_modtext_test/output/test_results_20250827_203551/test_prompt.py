...
def construct_prompt(reference_image_path, hard_negative_image_path, modification_text):
    ...
    - Reference(Picture 1): [REFERENCE] only describe the reference image(REFERENCE) in 2–4 literal sentences. Be thorough and accurate; list visible objects, attributes (color/material), counts, spatial relations, and readable text. Do not mention TARGET here. If unreadable, say “uncertain”.
    - Target(Picture 2): [TARGET] only describe the target image(TARGET) in 2–4 literal sentences. Same constraints; do not mention REFERENCE here.
    ...
    - you must only decompose the input edit text, do not refer to any information from the reference image or target image.
    - Format (one per line):
    ...
    - Answer each question (same IDs and order) with Yes/No/Uncertain and brief evidence based on the TARGET image.
    - Format (one per line):
...
### FEWSHOT EXAMPLE (CONTEXT ONLY — DO NOT COPY INTO OUTPUT),there are some examples that you can refer to.
##Example 1   
    <Input>
        "reference_image": <image_path>,
        "hard_negative_image": <image_path>,
...
##Example 3
    <Input>
        "reference_image": <image_path>,
        "hard_negative_image": <image_path>,
        "Input edit text": "Add a dog.",

    <Response>
    Generate Captions:
    Reference: [REFERENCE] two white puppies are playing in the snow.
    Target: [TARGET] four white dogs are playing on a snowy field.

    Decompose Intents:
    I1 | span="Add a dog" | intent="add a dog" | type=relative | objects=["dog"] | note="relative requirement"

    Generate Polar Questions:
    I1 | scope=compare_ref_target | Q="Comparing REFERENCE and TARGET, does the TARGET image have one more dog than the REFERENCE?"

    Answer Questions via Image Comparison:
    I1 | A=No | evidence_target="four white dogs are playing on a snowy field" | evidence_ref="two white puppies are playing in the snow" 

    Local Edits:
    I1 | action=rewrite | before="Add a dog" | after="Add two dogs" | reason="target has two more dogs than reference"

    Generate Text_new:
    text_new="Add two dogs"
...
def _parse_captions_block(block: str, result: dict):
    ...
    for line in lines:
        ...
        # 查找 Reference: [REFERENCE] ... 或 Target: [TARGET] ...
        ref_match = re.match(r'(?:-\s*)?Reference:\s*\[REFERENCE\]\s*(.+)', line, re.I)
        if ref_match:
            result["captions"]["reference"] = ref_match.group(1).strip()
            continue
            
        tgt_match = re.match(r'(?:-\s*)?Target:\s*\[TARGET\]\s*(.+)', line, re.I)
        if tgt_match:
...
def generate_response(model, processor, reference_image_path, hard_negative_image_path, modification_text, base_path, user_prompt: str = ""):
    ...
    try:
        ...
        content = [
            ...
            {"type": "text", "text": f'Input edit text: {modification_text}'}
        ]
